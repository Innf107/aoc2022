...
_start:
    ...
    push QWORD 0
    push QWORD 0
    mov r14, rsp ; r14 ^= pointer to the three maximum elves
.max_loop:
...
.sum_loop_done:
    ...
    cmp r13, [r14]  ; check if we exceeded max[0]
    jle .not_gt_1
    ; sum > max[0]
    mov rax, [r14+8]   ; shift the previous values over by one.
    mov [r14+16], rax  ; we discard [r14+16]
    mov rax, [r14]
    mov [r14+8], rax

    mov [r14], r13     ; insert the new value
    jmp .update_done

.not_gt_1:
    cmp r13, [r14+8]    ; check if we exceeded max[1]
    jle .not_gt_2
    ; sum < max[0] /\ sum > max[1]
    mov rax, [r14+8]    ; shift the previous value over by one
    mov [r14+16], rax 
    mov [r14+8], r13 
    jmp .update_done

.not_gt_2:
    cmp r13, [r14+16]   ; check if we exceeded max[2]
    jle .update_done
    ; sum < max[0] /\ sum < max[1] /\ sum > max[2]
    mov [r14+16], r13
